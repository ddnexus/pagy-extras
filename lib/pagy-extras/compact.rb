...
class Pagy
  ...
  module Frontend
    ...
    def pagy_nav_bootstrap_compact(pagy, id=caller(1,1)[0].hash)
      tags = '';
      link_marker = pagy_link_proc(pagy)
      link_prev = pagy_link_proc(pagy, 'class="link-prev btn btn-primary"')
      link_next = pagy_link_proc(pagy, 'class="link-next btn btn-primary"')

      tags << %(<nav id="pagy-nav-#{id}" class="pagy-nav-bootstrap-compact pagination" role="navigation" aria-label="pager">)

        tags << link_marker.call(MARKER, '', %(style="display: none;" ))

        tags << %(<div class="btn-group" role="group">)
        tags << (pagy.prev ? link_prev.call(pagy.prev, pagy_t('pagy.nav.prev'.freeze), 'aria-label="previous"'.freeze)
                           : %(<a class="link-prev btn btn-primary disabled" href="#">#{pagy_t('pagy.nav.prev'.freeze)}</a>))
        ...
        tags << (pagy.next ? link_next.call(pagy.next, pagy_t('pagy.nav.next'.freeze), 'aria-label="next"'.freeze)
                           : %(<a class="link-next btn btn-primary disabled" href="#">#{pagy_t('pagy.nav.next'.freeze)}</a>))
